...
class VanillaGradientDescent(BaseDescent):
    ...
    def calc_gradient(self, x: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        if self.loss_function is LossFunction.MSE:
            ...
            gradient = x.T @ np.tanh(x @ self.w - y) / x.shape[0]
        return gradient
...
class StochasticDescent(VanillaGradientDescent):
    ...
    def calc_gradient(self, x: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        if self.loss_function is LossFunction.MSE:
            ...
            x_subset = x[i]
            y_subset = y.iloc[i]
            predicted_values = self.predict(x_subset)
            prediction_error = predicted_values - y_subset
            ...
            predicted_values = self.predict(x_subset)
            prediction_error = predicted_values - y_subset
            gradient = -(x_subset.T @ np.tanh(prediction_error)) / self.batch_size
